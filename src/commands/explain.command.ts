...
WHAT IT IS -- two or three sentences. What role does this play in a DDD model, and when would a developer reach for it?

HOW TO USE IT -- if it is a base class, the contract you must satisfy to extend it and what the base gives you in return. If it is a collaborator, how the aggregate or value object delegates to it. If it is an interface, what implementing it commits you to.

EXAMPLE -- a short, realistic TypeScript snippet using only what the declaration shows. Use a plausible business domain, not Foo and Bar. Keep it under 25 lines.

WATCH OUT FOR -- one or two things that will actually bite someone. Base your answer on what the signatures imply: a protected constructor means static factories, an abstract member means the subclass must supply it, an options parameter means defaults exist that may not be what you want.
...
export class ExplainCommand extends CommandRunner {
  ...
  async run(args: string[], options: ExplainOptions = {}): Promise<void> {
    ...
    console.log(this.renderHeader(symbol));
    ...
    console.log(
      dim(`  Explaining with ${provider.id} (${provider.model})...\n`),
    );
    ...
    console.log(this.indent(explanation));
    console.log('');
  }

  /** The facts, printed before any model output so the two are distinguishable. */
  private renderHeader(symbol: StereotypeSymbol): string {
    const lines: string[] = [''];
    ...
    lines.push(
      `  ${dim(symbol.file)}   ${cyan(symbol.family)}   ${yellow(symbol.role)}`,
    );

    if (symbol.aliasOf) {
      lines.push('');
      lines.push(
        `  ${yellow('Alias for')} ${bold(symbol.aliasOf)}${dim(
          ' -- the two names refer to the same class.',
        )}`,
      );
    }
    ...
    if (symbol.doc) {
      lines.push('');
      lines.push(this.indent(symbol.doc, '  '));
    }

    if (symbol.abstractMembers.length) {
      lines.push('');
      lines.push(`  ${green('You must implement:')}`);
    ...
    if (statics.length) {
      lines.push('');
      lines.push(`  ${green('Static factories:')}`);
      for (const member of statics) {
        lines.push(`    ${dim(member.signature)}`);
      }
    }
